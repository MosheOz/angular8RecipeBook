...
export class AuthService {
    ...
    private handleAuthentication(email: string, userId: string, token: string, dateExp: number) {
        const expirationDate = new Date(new Date().getTime() + dateExp * 1000);
        const user = new User(email, userId, token, expirationDate);
        this.user.next(user);
        this.autoLogout(dateExp * 1000);
